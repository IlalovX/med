...
function DirectorSimulationCenter() {
  return (
    <div className="container mx-auto p-5 space-y-5">
      <h1 className="text-2xl sm:text-4xl font-bold">Сортудники центра</h1>
      <div className="space-y-10">
        {STRUCTURE.map((item, index) => (
          <div key={index} className="flex gap-x-10">
            <img src={image} />
            <div className="relative">
              <h3 className="text-2xl sm:text-4xl font-bold">{item.name}</h3>
              <h4 className="text-xl sm:text-3xl">{item.profession}</h4>
