function LifeStudents() {
  return (
    <div className="container mx-auto p-5 space-y-10">
      <h1 className="text-2xl sm:text-4xl font-bold">Студенческая общежития</h1>
      <p className="text-xl sm:text-2xl">
        Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ea, eaque
        quae? Vel aliquam odit odio error. Sint nisi ipsum ea repellat pariatur
        dignissimos voluptatem, vitae at impedit voluptatum exercitationem!
        Quaerat.
      </p>
      <div className="w-full bg-slate-700 rounded-lg min-h-[182px] sm:min-h-[520px]"></div>
      <div className="w-full bg-slate-700 rounded-lg min-h-[182px] sm:min-h-[520px]"></div>
    </div>
  );
}

export default LifeStudents;
